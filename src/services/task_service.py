...
class TaskService:
    ...
    def get_all_tasks(self) -> List[Task]:
        """获取所有顶层任务（不含分任务、不含已删除），按创建时间倒序"""
        return (
            self._session.query(Task)
            .filter(Task.parent_id.is_(None), Task.is_deleted != 1)
    ...
    def delete_task(self, task_id: int) -> bool:
        ...
        for sub in self.get_subtasks(task_id):
            ...
            related_ids.append(sub.id)
        self._unlink_notes_for_tasks(related_ids)
        self._session.commit()
        return True
    ...
    def permanent_delete_task(self, task_id: int) -> bool:
        ...
        if task is None:
            return False
        related_ids = [task.id] + [s.id for s in self.get_subtasks(task.id)]
        self._unlink_notes_for_tasks(related_ids)
    ...
    def carry_forward(self, task_id: int) -> Optional[Task]:
        ...
        2. 将分任务、关联便签等全部转移到新任务
        3. 原任务保留在原日期，状态改为done（这样历史回顾中仍能看到）
        """
        ...
        self._session.flush()  # 获取 new_task.id
        
        # 转移分任务：将 parent_id 指向新任务
        for sub in self.get_subtasks(task_id):
            sub.parent_id = new_task.id
        
        # 转移关联便签：将 note.task_id 指向新任务
        linked_notes = (
    ...
    def _unlink_notes_for_tasks(self, task_ids: list[int]):
        if not task_ids:
            return
        (
            self._session.query(Note)
            .filter(Note.task_id.in_(task_ids))
    ...
    def get_task_link_count(self, task_id: int) -> int:
        """返回任务关联的未删除便签数量。"""
        linker = LinkService(self._session)
        return linker.count_notes_for_task(task_id)
