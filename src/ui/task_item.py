...
class SubtaskItemWidget(QWidget):
    ...
    def contextMenuEvent(self, event):
        ...
        del_action = menu.addAction("删除分任务")
...
class TaskItemWidget(QFrame):
    ...
    def _setup_ui(self):
        ...
        if self._is_carryover:
            ...
            self._link_badge.setObjectName("TaskLinkBadge")
            self._link_badge.setSizePolicy(QSizePolicy.Fixed, QSizePolicy.Preferred)
            self._update_link_badge()
            right_box_layout.addWidget(self._link_badge)
    ...
    def _update_link_badge(self):
        if self._link_badge is None:
            return
        if self._link_count <= 0:
            self._link_badge.setVisible(False)
            return
        self._link_badge.setPixmap(make_link_pixmap(14))
        self._link_badge.setVisible(True)

    def set_link_count(self, count: int):
        self._link_count = max(0, int(count or 0))
        self._update_link_badge()
    ...
    def contextMenuEvent(self, event):
        ...
        create_link_action = menu.addAction("新建关联便签")
        create_link_action.triggered.connect(
            lambda: self.create_linked_note_requested.emit(self.task.id)
        )

        link_existing_action = menu.addAction("关联已有便签")
        link_existing_action.triggered.connect(
            lambda: self.link_existing_note_requested.emit(self.task.id)
        )

        view_link_action = menu.addAction("打开关联便签")
        view_link_action.triggered.connect(
            lambda: self.view_linked_notes_requested.emit(self.task.id)
        )
