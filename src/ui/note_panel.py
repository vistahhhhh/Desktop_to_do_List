...
from PyQt5.QtGui import QColor, QFont, QFontMetrics, QTextCharFormat, QTextBlockFormat, QTextCursor
...
class NotePanel(QWidget):
    ...
    def _make_list_row(self, note):
        ...
        row.setMinimumWidth(80)  # 设置合理的最小宽度：日期40px + 删除按钮22px + 少量间距
        hl = QHBoxLayout(row)
        ...
        row.mousePressEvent = lambda e, n=note: self._select_from_list(n)
    ...
    def _select_from_list(self, note):
        self._flush_save()
    ...
    def _auto_save(self):
        ...
        has_content = bool(title) or bool(plain)

        if self._current_note is None:
            # 当前无便签，只有有内容时才创建
            if has_content:
                self._current_note = self._svc.create()
                self._svc.save(self._current_note, title, body_html)
        else:
            if has_content:
                # 已有便签，正常保存
                self._svc.save(self._current_note, title, body_html)
