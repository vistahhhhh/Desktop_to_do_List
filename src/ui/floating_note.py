...
class FloatingNoteWindow(QWidget):
    ...
    def __init__(self, note_service, note_id: int, stylesheet: str = "", parent=None):
    ...
    def _load_note(self):
        if not self._note:
            return
        self._loading = True
        self.title_edit.setText(self._note.title or "")
    ...
    def _auto_save(self):
        ...
        self._svc.save(fresh, title, body_html)
        self._note = fresh
    ...
    def closeEvent(self, event):
        if self._auto_save_timer.isActive():
            self._auto_save_timer.stop()
        self._auto_save()
        # 发送关闭信号，通知主窗口（若该便签未被“空内容删除”）
        if self._note_id is not None:
            self.closed.emit(self._note_id)
        event.accept()
