...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.link_service = LinkService(self._session)
        self.link_service.unlink_past_daily_tasks()
    ...
    def _setup_ui(self):
        ...
        self.task_list.batch_delete_requested.connect(self._on_task_batch_delete)
        ...
        self.note_panel.manage_links_requested.connect(self._on_manage_note_links)
    ...
    def _refresh_tasks(self):
        ...
        for task in tasks:
            ...
            link_count_map[task.id] = self.task_service.get_task_link_count(task.id)
    ...
    def _on_create_linked_note(self, task_id: int):
        try:
            task = self.task_service.get_task(task_id)
            if task is None:
                self._show_tip("目标任务不存在")
                return

            existed = self.link_service.get_note_for_task(task_id, include_deleted=True)
            if existed is not None:
                if not self._show_confirm("已有关联便签，是否替换？"):
                    return
                old_note_id = existed.id
                self.link_service.unbind_by_task(task_id)
                self._on_note_unlinked(old_note_id)

            title_hint = f"[{task.title}]"
            note = self.note_service.create_for_task(task_id, title_hint=title_hint)
            if note is None:
                self._show_tip("创建关联便签失败")
                return

            # 让新便签成为当前便签，便签列表中会默认排在原便签前面
            self.note_panel.load_note_by_id(note.id)
    ...
    def _on_link_existing_note(self, task_id: int):
        task = self.task_service.get_task(task_id)
        if task is None:
            self._show_tip("目标任务不存在")
            return

        # 如果已有关联便签，弹出"打开/替换"选项
        existed = self.link_service.get_note_for_task(task_id, include_deleted=True)
        if existed is not None:
            choice = self._show_open_or_replace("该任务已有关联便签，请选择操作：")
            if choice == "open":
                self._open_note_by_id(existed.id)
                return
            elif choice != "replace":
                return
            # choice == "replace"：继续弹出选择列表

        notes = self.note_service.get_all()
        if not notes:
            self._show_tip("暂无可关联便签")
            return

        dialog = LinkExistingNotesDialog(task.title, notes, parent=self)
        dialog.setStyleSheet(self.styleSheet())
        ...
        replace = existed is not None

        target_note = self.note_service.get_by_id(note_id)
        if target_note is not None and target_note.task_id not in (None, task_id):
            if not self._show_confirm("该便签已关联其他任务，是否改绑到当前任务？"):
                return
            replace = True

        # 记录旧关联便签（用于清除前缀）
        old_linked = self.link_service.get_note_for_task(task_id) if replace else None

        if self.link_service.link(task_id, note_id, replace=replace):
            # 清除旧便签的 [] 前缀
            if old_linked is not None and old_linked.id != note_id:
                self._on_note_unlinked(old_linked.id)
            # 更新新便签标题 [] 前缀
    ...
    def _on_view_linked_notes(self, task_id: int):
        ...
        linked = self.link_service.get_note_for_task(task_id)
        if linked is None:
            self._show_tip("当前任务暂无关联便签")
            return
        self._open_note_by_id(linked.id)

    def _on_manage_note_links(self, note_id: int):
        from datetime import date
        note = self.note_service.get_by_id(note_id)
        if note is None or getattr(note, "deleted", False):
            self._show_tip("当前便签不存在")
            return

        today = date.today()
        all_tasks = [
            t for t in self.task_service.get_all_tasks()
            if getattr(t, 'is_deleted', 0) != 1
            and not (
                getattr(t, 'task_type', '') == 'short_term'
                and getattr(t, 'task_date', None) is not None
                and t.task_date < today
            )
        ]
        dialog = NoteLinkedTasksDialog(note_id, self.link_service, all_tasks=all_tasks, parent=self)
        dialog.setStyleSheet(self.styleSheet())
        dialog.jump_task_requested.connect(self._jump_to_task)
    ...
    def _on_note_unlinked(self, note_id: int):
        ...
        if bare != title:
            self.note_service.save(note, bare, note.body_html)

        # 检查便签是否无实际内容（标题为空 + 正文为空）
        body_plain = re.sub(r'<[^>]+>', '', note.body_html or '').strip()
        body_plain = re.sub(r'&[a-zA-Z]+;', '', body_plain).strip()
        if not bare.strip() and not body_plain:
            # 关闭对应的浮动便签窗口
            for win in list(self._floating_notes):
                if getattr(win, '_note_id', None) == note_id:
                    win._note_id = None
                    win.close()
            # 若便签面板当前显示的就是这个空便签，切到新空白
            if hasattr(self, 'note_panel') and self.note_panel.get_current_note_id() == note_id:
                self.note_panel.start_new_note()
            self.note_service.permanent_delete(note_id)
            return
        ...
        for win in self._floating_notes:
            if getattr(win, '_note_id', None) == note_id:
                ...
                win.title_edit.setText(bare)
                win._loading = False
        # 刷新便签面板标题
        if hasattr(self, 'note_panel') and self.note_panel.get_current_note_id() == note_id:
            self.note_panel._loading = True
            self.note_panel.set_title_prefix("")
            self.note_panel.title_edit.setText(bare)
            self.note_panel._loading = False

    def _on_note_linked(self, note_id: int, task_id: int):
        """便签关联任务后，自动设置/更新标题中的 [任务名] 前缀"""
        import re
        note = self.note_service.get_by_id(note_id)
        task = self.task_service.get_task(task_id)
        if note is None or task is None:
            return
        title = note.title or ""
        # 去除旧前缀（如有）
        bare = re.sub(r'^\[.*?\]\s*', '', title)
        prefix = f"[{task.title}]"
        new_title = f"{prefix} {bare}" if bare else prefix
        self.note_service.save(note, new_title, note.body_html)
        # 同步更新已打开的浮动便签窗口
        for win in self._floating_notes:
            if getattr(win, '_note_id', None) == note_id:
                win._loading = True
                win.set_title_prefix(prefix)
                win.title_edit.setText(bare)
                win._loading = False
        # 刷新便签面板标题
        if hasattr(self, 'note_panel') and self.note_panel.get_current_note_id() == note_id:
            self.note_panel._loading = True
            self.note_panel.set_title_prefix(prefix)
            self.note_panel.title_edit.setText(bare)
            self.note_panel._loading = False

    def _jump_to_task(self, task_id: int):
        task = self.task_service.get_task(task_id)
        if task is None:
            self._show_tip("目标任务不存在")
            return
        self._switch_to_todo()
    ...
    def _on_task_batch_delete(self, task_ids: list):
        """批量删除任务"""
        count = len(task_ids)
        if count == 0:
            return
    ...
    def _show_open_or_replace(self, message: str) -> str:
        """显示'打开/替换'选择弹窗，返回 'open' / 'replace' / 'cancel'"""
        dialog = QDialog(self)
        ...
        open_btn.setObjectName("EditorSaveBtn")
        ...
        replace_btn.setObjectName("EditorSaveBtn")
        replace_btn.setCursor(Qt.PointingHandCursor)
        replace_btn.setFixedSize(80, 34)
        def _on_replace():
            _result[0] = "replace"
            dialog.accept()
        replace_btn.clicked.connect(_on_replace)
