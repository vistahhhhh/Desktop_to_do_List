...
from src.ui.tag_sidebar import (
    TagSidebar,
    TagButton,
    SMART_LISTS,
...
def test_sidebar_creation():
    sidebar = TagSidebar()
    assert sidebar.width() == SIDEBAR_COLLAPSED_WIDTH
...
def test_task_item_display():
    ...
    assert hasattr(item, "create_linked_note_requested")
    assert hasattr(item, "link_existing_note_requested")
    assert hasattr(item, "view_linked_notes_requested")
    session.close()
    print("[PASS] test_task_item_display - 任务项显示正确")


def test_task_item_link_badge_display():
    session = _make_session()
    svc = TaskService(session)
    task = svc.create_task("关联徽标测试", task_type=Task.TYPE_SHORT_TERM)

    item = TaskItemWidget(task, link_count=3)
    assert item._link_badge is not None
    assert not item._link_badge.isHidden()
    assert item._link_badge.text() == "📎"

    item.set_link_count(0)
    assert item._link_badge.isHidden()
    session.close()
    print("[PASS] test_task_item_link_badge_display - 关联徽标显示正确")
...
def run_all():
    tests = [
        ...
        test_task_item_link_badge_display,
